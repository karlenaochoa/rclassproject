...
## Participants
  Children and parents were recruited by the Developing Mind Laboratory at the Psychology Department at University of Oregon. One-hundred three-(M age) and four-year-olds (M age) participated and were randomly assigned to either the first-hand condition (N ) or second-hand condition (N). 
...
df_aov <- df %>% 
  group_by(age_group) %>% 
  summarize(mean(average_trust))
papaja::apa_table(df_aov)
```
...
cor.test(summary_plot2$average_csus, summary_plot2$average_trust )
...
cor.test(summary_plot3$average_EF, summary_plot3$average_trust)
```
